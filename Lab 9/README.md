...
This project demonstrates a fully observable Spring Boot application using OpenTelemetry, Prometheus, Tempo, Loki, and Grafana.

## Structure
- **app/**: Spring Boot application source code.
- **infra/**: Infrastructure configuration (Docker Compose, Config files).
- **infra/config/**: Configuration files for the observability components.

## How to Run

### 1. Run Full Stack (App + Infra)
To build the application and start everything:
```bash
./run.sh
```
This will:
- Build the Spring Boot application (Maven)
- Build the Docker image with the OpenTelemetry Java Agent
- Start all services (Collector, Prometheus, Tempo, Loki, Grafana, App)

Access the App: http://localhost:8080/hello
Access Grafana: http://localhost:3000

```bash
./call.sh
```
Don't forget to stop the calls with `Ctrl+C`.

### 2. Run Infrastructure Only (Development Mode)
If you want to run the infrastructure in Docker but run the app locally (e.g. in your IDE):
```bash
./run-infra.sh
```
Then, run your app locally. *Note: To send telemetry from your local app to the dockerized collector, you'll need to configure the OTLP endpoint to point to `localhost:4317` and ensure the agent is attached.*

## Components
- **App**: Spring Boot 3.4.1 (Java 21)
- **Agent**: OpenTelemetry Java Auto-instrumentation
